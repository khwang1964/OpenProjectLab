...
# 此檔案位於：
# F:\OpenProjectLab\generator\cli\main.py
#
# parents[0] -> F:\OpenProjectLab\generator\cli
# parents[1] -> F:\OpenProjectLab\generator
# parents[2] -> F:\OpenProjectLab
...
def build_parser() -> argparse.ArgumentParser:
    ...
    subparsers = parser.add_subparsers(
        dest="command",
        metavar="{list,doctor,bootstrap,course,week}",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # 支援舊版 --list (legacy compatibility)，
    # 正式介面為 `opl list`。
    if args.legacy_list or args.command in (None, "list"):
        print("\n".join(REGISTRY.names()))
        return 0

    if args.command == "doctor":
        checks = run_doctor(args.config, PROJECT_ROOT)
        for check in checks:
