...
The English and Traditional Chinese (Taiwan) User Manuals now document the
same production AI CLI command inventory and execution boundaries without
changing runtime behavior.

The bilingual parity contract covers:

- exact `opl ai course`, `review`, `document`, and `template` inventory;
- Stable deterministic local-response execution;
- Experimental explicit / injection-only / fail-closed provider execution;
- exit-code 2, stderr diagnostics, and no-success-output failure semantics;
- non-mutating filesystem and repository behavior;
- explicit rejection of automatic SDK import, automatic credential lookup,
  implicit provider selection, and network fallback.

The local documentation regression passed 80 tests and pre-commit passed.
This evidence authorizes the documentation PR, but it does not pre-accept
v1.1.6.10, the broader AI CLI implementation, or Formal v1.1 Acceptance.
...
- [x] Existing documentation regression passes.
- [x] AI CLI Implementation Acceptance remains Not Accepted.
- [x] Formal v1.1 Acceptance remains Not Accepted.
