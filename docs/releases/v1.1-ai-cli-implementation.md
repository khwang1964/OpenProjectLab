...
This gate documents the production AI CLI surface in both formal User Manuals
without changing runtime behavior. English and Traditional Chinese (Taiwan)
documentation must describe the same command inventory, stable local-response
path, Experimental provider boundary, validation/failure semantics, and
non-mutating behavior before implementation acceptance can proceed.
