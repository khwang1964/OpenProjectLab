from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
ARCH = ROOT / "docs/architecture/deterministic-readiness-snapshot-comparison.md"
RELEASE = ROOT / "docs/releases/v1.4.2-deterministic-readiness-snapshot-comparison-design.md"


def normalized(path: Path) -> str:
    return " ".join(path.read_text(encoding="utf-8").split())


def test_design_is_pending_and_implementation_not_started() -> None:
    assert "Proposed / Pending design review" in normalized(ARCH)
    assert "Proposed / Pending design review" in normalized(RELEASE)
    assert "Production implementation is Not Started" in normalized(RELEASE)


def test_comparison_is_deterministic_and_fail_closed() -> None:
    text = normalized(ARCH)
    for phrase in (
        "UNCHANGED",
        "IMPROVED",
        "REGRESSED",
        "INCOMPARABLE",
        "stable reason codes",
        "before/after values",
        "cannot report improvement",
    ):
        assert phrase in text


def test_all_readiness_gates_are_compared() -> None:
    text = normalized(ARCH)
    for phrase in (
        "required checks",
        "focused and regression counts",
        "coverage",
        "audit-chain acceptance",
        "supported surfaces",
        "limitations",
        "release blockers",
    ):
        assert phrase in text


def test_cli_is_bounded_offline_and_read_only() -> None:
    text = normalized(ARCH)
    assert "readiness compare --baseline BASELINE --candidate CANDIDATE" in text
    assert "--format json|text" in text
    assert "aggregate-byte limits" in text
    assert "Exit 0" in text and "exit 1" in text and "exit 2" in text
    assert "no discovery or mutation" in text


def test_authority_boundary_is_explicit() -> None:
    text = normalized(ARCH)
    for phrase in (
        "does not prove Git ancestry",
        "release approval",
        "compatibility promotion",
        "cannot waive readiness gates",
        "access credentials or networks",
        "publication state",
    ):
        assert phrase in text


def test_governance_markers_are_exact_and_unique() -> None:
    base = "v1.4.2-deterministic-readiness-snapshot-comparison-design"
    for relative, suffix in (
        ("CHANGELOG.md", "changelog"),
        ("docs/HISTORY.md", "history"),
        ("docs/roadmap.md", "roadmap"),
    ):
        text = (ROOT / relative).read_text(encoding="utf-8")
        assert text.count(f"<!-- {base}-{suffix} -->") == 1
        assert "Production implementation — Not Started" in text
